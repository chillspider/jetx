...
const WashFailedDialog: React.FC<Props> = ({ onConfirm, onGoSupport, isVisible, ...props }) => {
	const {
	...
	return (
		<View>
			<Modal {...props} animationType="fade" visible={isVisible}>
				<View style={styles.modal}>
					<Box
						...
						p={spacing.xl}
					>
						<Text style={styles.title}>{t('process.failed')}</Text>
						<Box height={24} />
						<IcError />
						<Box height={8} />
						<Text style={styles.noti}>{t('process.failedNoti')}</Text>
						<Button containerStyle={styles.button} title={t('know')} onPress={onConfirm} />
						<Pressable style={styles.callButton} hitSlop={12} onPress={onGoSupport}>
							<CallCenterIcon />
...
export default WashFailedDialog;
