...
export function ApiFile(
  ...
  let apiFileInterceptors: MethodDecorator[] = [];
  ...
  if (filesArray.length === 1) {
    ...
    if (file.isArray) {
      apiFileInterceptors = [
        UseInterceptors(FilesInterceptor(file.name, file.maxCount)),
      ];
    } else {
      apiFileInterceptors = [UseInterceptors(FileInterceptor(file.name))];
    }
  } else if (filesArray.length) {
    const uploadFields = filesArray.map((file) => ({
      name: file.name,
