...
export const encryptJson = async (data: object) => {
	try {
		const encrypted = await encrypt(JSON.stringify(data), secretKey);
		return encrypted;
	} catch (_) {
		return null;
	}
...
export const decryptJson = async <T>(data: string) => {
	if (!data) return null;

	try {
		const decrypted = await decrypt<T>(data, secretKey);
		return decrypted;
	} catch (_) {
		return null;
	}
